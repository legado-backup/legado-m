...
> 迁移自 [modules/service-layer.md](../modules/service-layer.md) 原 §9「Python 重构参考」（2026-08-30 拆分）。本文件为服务层 Python 重构实现的唯一权威源：9.1 下载服务、9.2 缓存服务、9.3 书源检测服务、9.4 WebService、9.5 TTS 朗读、9.6 音频播放、9.7 导出书籍、9.8 服务层数据流总结。
> Android 侧实现见 [modules/service-layer.md](../modules/service-layer.md)。
> 小节编号（9.1-9.8）保留自原文件，便于溯源。
