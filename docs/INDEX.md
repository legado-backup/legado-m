...
- [~~主界面底栏精简~~](./specs/archive/2026-09-14-main-bottom-nav-simplify/README.md) - 默认底栏移除「统计」Tab（书架/发现/订阅/我的四项）+ 阅读统计仪表盘迁至「我的-工具-阅读记录」子页（新增 ReadRecordStatsActivity 复用 ReadRecordFragment）+ 主题/底栏设置体系联动移除统计配置（MainBottomNavConfig.specs/NavigationBarIconConfig.items 链式清理）+ floating 底栏右下搜索框默认隐藏（默认值链翻转 4 处，自定义套装编辑能力保留）+「我的-工具」移除订阅源全局搜索入口（RssSearchActivity 与订阅 Tab 顶栏搜索保留）✅ **已完成并归档**（2026-09-14 用户验收通过；L1 编译过 + MEmu L2 ①-④实测 + Grep 0 残留；测试包 3.26.091410）

- [体检收编校验所选](./specs/quality-check-unify/README.md) - 源质量体检 vs 校验所选全方位对比（检测维度超集/效率/容错均优，缺结果落库）+ 替代评估决策（用户选方案1）；QualityReportApplier 桥接层（report→BookCheckResult/RssCheckResult 纯映射+失效分组逐字回填+weight 复用 SourceWeightCalculator+suspect 防误伤+wSourceComment 注释）+ 体检结果页底栏「应用结果」（AppConfirmDialog 确认+RUNNING 禁用+toast 反馈）；单测 8 绿（NOT_APPLICABLE 已检语义对齐旧服务），包 3.26.091323；校验所选入口本版保留，下一版本清理 ✅ L1 完成，⏳ L2 真机验收
- [导入源质量过滤与公共源质量校验组件](./specs/import-source-quality-filter/README.md) - 校验抽象公共组件 SourceQualityChecker（剥离 CheckSourceService 探测核心，零写库副作用，ProbeOptions 统一探测参数）三场景分流：①导入过滤（L1 静态+L2 联网试采+L3 深度，三档严格度确定性证据过滤，suspect 五类强制放行防误伤+防内网扫描，FILTERED 复核窗口/全选二次确认/导入即禁用/返回键半程导入，默认关闭）②存量源质量体检（四态白话筛选+得分排序+禁用主按钮+删除自动 JSON 备份+会话按类型分+暂停续跑）③现有校验服务等价重构（AD-08 防走样清单 6 点）；评分 v2 形态模板（书源4/订阅源3）+已校验维度归一化+三态；千条性能（Semaphore/host 缓存/试采限额/HEAD 截断防拖库/断网预检短路/结论缓存 TTL24h）；**全链路诊断日志 tag=QualityCheck（启动/进度里程碑/异常源带失败维度/流式落库/暂停续跑/全选反选禁用删除点击）**；UI 归位主题体系（结果页 palette 直色+LegadoMiuixActionButton，43 处 colorScheme 违例清零含投屏面板/调试页/弹层壳）；全选反转真 bug 修复（onSelectAll 分支写反）🔄 开发中（暗色适配+全选修复包 091321/091322 已交付，待真机复测）
- [内置视频播放器 DLNA/UPnP 投屏](./specs/add-dlna-cast/README.md) - 播放器下拉菜单新增「投屏」：SSDP 发现局域网 MediaRenderer + 自研轻量 UPnP 客户端（零新依赖，否决 Cling/JUPnP/media3-cast）+ 三态媒体投递（直投 / 防盗链头代理 / file:// 本地文件代理）+ `CastProxyServer`（NanoHTTPD 临时端口，Range 全形态 + HEAD + m3u8 清单重写 + token 白名单）+ `DlnaCastService` 前台保活（mediaPlayback，避 Android 15 dataSync 6h 上限）+ 投屏即暂停本地播放（禁第二路拉流）；15 条 ADR、14 条 REQ、7 条 E2E，零 DB 迁移、零依赖变更、+1 权限；**红队自审 5 轮 22 项 + 红蓝双代理对抗 5 轮 23 项；NanoHTTPD API 经 javap 实证**；假渲染器 `ai_tests/scripts/dlna_fake_renderer.py` 已就绪（自检 10/10）🔄 开发中（检查点 1 已通过 2026-09-12，L2 定真机 + PC 同 Wi-Fi）
