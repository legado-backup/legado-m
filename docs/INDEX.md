...
| [specs/ui-redesign-m3/](./specs/ui-redesign-m3/) | UI 重构设计（深度对标四仓前端：HapeLee MD3全Compose组件�?Rimchars AppSetting组件�?legado_NG 双端Token系统+Glassmorphism/鸿蒙版三级布局 + MoRealm墨境纯Compose现代工程标杆：主�?色→34槽位推导/PillNavigationBar悬浮胶囊底栏/设置三模�?shimmer骨架屏；整体前端思想综合收敛五支�?五仓贡献矩阵+功能不裁剪红线清单（A内核/B数据/C UI入口/D死代码）；实现细化规格（17组件签名/主题toM3Scheme映射/真实文件锚点/PR+KPI/themeConfig格式封口）；组件化优�?公共组件�?MyCenter 统计卡→高频卡→低频列表；保留暗夜紫默认主题；交付每页四要素设计文档+ADR 01-18，本次仅设计不写代码�?🔄 设计�?|
