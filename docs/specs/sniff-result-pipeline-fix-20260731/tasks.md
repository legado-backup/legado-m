...
- [ ] 读取 `VideoUrlExtractor.kt` L491-572 确认 extractVideoUrlForEpisode 当前实现
- [ ] 读取 `BackstageWebView.kt` L332-364 确认 shouldInterceptRequest 当前实现
- [ ] 读取 `HttpHelper.kt` L75-167 确认 okHttpClient 当前配置
- [ ] 读取 `OkHttpStreamFetcher.kt` L57-181 确认 onFailure/onResponse 当前实现
- [ ] 读取 `CronetInterceptor.kt` L75-103, L170-180, L300-340 确认 lastFailedHostHint 当前逻辑
- [ ] 读取 `DohDns.kt` L58-66 确认 DOH_SERVERS 当前配置
- [ ] 备份上述 6 个文件到 `app/src/main/java/io/legado/app/.bak/sniff-result-pipeline-fix/`

### T1.2 编译环境验证

- [ ] 确认 `gradlew.bat assembleAppDebug` 可正常编译（基线）
- [ ] 确认测试包 `io.legado.miss.app.debug` 可正常安装
...
- [ ] 修改 `VideoUrlExtractor.kt` L505-572
...
- [ ] Grep 确认 `withTimeoutOrNull(12000L)` 已移除
- [ ] 编译验证：`gradlew.bat assembleAppDebug` BUILD SUCCESSFUL

### T2.2 FR-2: R5 抓包命中后切 UI 线程同步 resume

- [ ] 修改 `BackstageWebView.kt` L346-360
  - 将 `callback?.onResult(response)` 移入 `mHandler.post { }` 块
  - 将 `destroy()` 也移入同一 `mHandler.post { }` 块
  - 修改日志："R5网络抓包命中(工作线程)" → "R5网络抓包命中(切UI线程)"
- [ ] Grep 确认 `callback?.onResult(response)` 在 `mHandler.post` 内
- [ ] 编译验证：`gradlew.bat assembleAppDebug` BUILD SUCCESSFUL

### T2.3 P0 阶段编译验证

- [ ] 编译测试包：`gradlew.bat assembleAppDebug`
- [ ] 编译正式包：`gradlew.bat assembleAppRelease`
- [ ] mapping.txt 验证：5 个关键类全部保留（GEN_JNI/CronetLibraryLoaderJni/RedirectCacheInterceptor/CronetInterceptor/DohDns）
- [ ] 模拟器安装启动验证：App 不崩溃
...
- [ ] 创建 `app/src/main/java/io/legado/app/help/http/StreamResetRetryInterceptor.kt`
  - 实现 `Interceptor` 接口
...
- [ ] 修改 `HttpHelper.kt` L154 后新增 `builder.addInterceptor(StreamResetRetryInterceptor)`
- [ ] Grep 确认 `StreamResetRetryInterceptor` 已添加到 okHttpClient
- [ ] 编译验证：`gradlew.bat assembleAppDebug` BUILD SUCCESSFUL

### T3.2 FR-3: OkHttpStreamFetcher StreamReset 识别

- [ ] 修改 `OkHttpStreamFetcher.kt` L130-141
...
- [ ] Grep 确认 `isStreamResetException` 已添加
- [ ] 编译验证：`gradlew.bat assembleAppDebug` BUILD SUCCESSFUL
...
- [ ] 编译测试包：`gradlew.bat assembleAppDebug`
- [ ] 编译正式包：`gradlew.bat assembleAppRelease`
- [ ] mapping.txt 验证：StreamResetRetryInterceptor 类保留
- [ ] 模拟器安装启动验证：App 不崩溃
...
- [ ] 读取 `ai_tests/docs/fixed_test_workflow.md` SOP
- [ ] 使用 `ai_tests/venv/Scripts/python.exe` 执行 `ai_tests/scripts/quick_build_install.py`
- [ ] 测试包 `io.legado.miss.app.debug` 安装到真机/模拟器
- [ ] L1 验证：App 启动不崩溃
- [ ] L2 验证：
  - 打开视频订阅源，播放视频
...
- [ ] 记录所有测试中发现的问题到 `docs/issues/found/issues-found.md`
- [ ] 每个问题包含：现象、日志行号、根因分析、修复方案
...
- [ ] 编译正式包：`gradlew.bat assembleAppRelease`
- [ ] 验证 APK 大小和包名（`io.legado.miss.app.release`）
- [ ] mapping.txt 验证：
...
- [ ] 更新 `.trae/memory/ai_memory_main.md`：
  - 当前任务状态
  - 用户反馈记录
  - 经验沉淀
...
- [ ] 正式包 APK 交付到 `output/apk/release/`
- [ ] 使用 AskUserQuestion 询问用户真机验证
...
- [ ] `VideoUrlExtractor.kt` 不含 `withTimeoutOrNull(12000L)`
- [ ] `BackstageWebView.kt` 的 `callback?.onResult` 在 `mHandler.post` 内
- [ ] `HttpHelper.kt` 含 `StreamResetRetryInterceptor`
- [ ] `OkHttpStreamFetcher.kt` 含 `isStreamResetException` 方法
- [ ] `CronetInterceptor.kt` 含 `lastFailedHostHintTimeMs` 和 `HINT_TIMEOUT_MS`
- [ ] `DohDns.kt` 的 `DOH_SERVERS` 只含 2 个服务器
- [ ] Grep 确认无调试日志残留（`android.util.Log.d` / `android.util.Log.e`）

### 编译验证检查

- [ ] 测试包 BUILD SUCCESSFUL
- [ ] 正式包 BUILD SUCCESSFUL
- [ ] mapping.txt 关键类全部保留

### 真机测试检查

- [ ] 无"extractVideoUrlForEpisode timeout (12s)"日志
...
- [ ] `assets/updateLog.md` 已更新
- [ ] `.trae/memory/ai_memory_main.md` 已更新
- [ ] `docs/issues/found/issues-found.md` 已记录问题（如有）
