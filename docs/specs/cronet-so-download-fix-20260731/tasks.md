...
- [ ] 1.1 阅读真机日志分析报告（docs/issues/user/temp/20260731/001/extracted_v4/logs/appLog-26-07-31_09-12-34.018.txt）
- [ ] 1.2 阅读现有源码（DohDns.kt / CronetLoader.kt / CronetInterceptor.kt / CronetHelper.kt / ExoPlayerHelper.kt）
- [ ] 1.3 备份待修改文件到 .bak 目录（含 DohDns.kt / CronetLoader.kt / CronetInterceptor.kt / CronetHelper.kt / ExoPlayerHelper.kt / proguard-rules.pro）
- [ ] 1.4 确认 GitHub Releases 上传 libcronet.so 资产（如未上传需先上传）
- [ ] 1.5 阅读历史设计文档关键章节（player-mature-solutions-alignment Phase 4.2 302 缓存 / video-player-m3u8-fix AES-128 密钥注入）

## 2. DoH 服务器配置修复（维度1，P0，FR-01）

- [ ] 2.1 修改 `app/src/main/java/io/legado/app/help/http/DohDns.kt` 的 `DOH_SERVERS` 列表
...
- [ ] 2.2 验证 DohDns 服务器配置正确性（URL 格式、bootstrap IP）
- [ ] 2.3 真机日志确认 DoH 解析成功率 > 80%

## 3. SO 下载源切换 + 下载逻辑修复（维度2+3，P0，FR-02/03）

- [ ] 3.1 修改 `app/src/main/java/io/legado/app/lib/cronet/CronetLoader.kt` 的 `soUrl` 初始化
...
- [ ] 3.2 修复 `downloadFileIfNotExist` 函数逻辑
  - 文件存在时校验 md5（传入 md5 参数）
  - md5 不匹配时删除文件重新下载
  - 重命名为 `downloadFile`（语义更准确）或保留原名增加 md5 参数
- [ ] 3.3 更新 `app/src/main/assets/cronet.json` 配置（如需更新 md5 或 URL）
- [ ] 3.4 验证 GitHub Releases 上 libcronet.so 资产可访问（HEAD 请求确认 200）
- [ ] 3.5 真机首次安装验证 SO 从 GitHub Releases 下载成功（md5 匹配）

## 4. Cronet HTTP/2 错误降级优化（维度5，P1，FR-04）

- [ ] 4.1 修改 `app/src/main/java/io/legado/app/lib/cronet/CronetInterceptor.kt` 的降级策略
...
- [ ] 4.2 验证降级逻辑正确性（错误类型识别、降级时长）
- [ ] 4.3 真机日志确认 HTTP/2 错误后 1 分钟自动恢复探测

## 5. 嗅探超时恢复（维度8，P1，FR-05）

- [ ] 5.1 修改 `app/src/main/java/io/legado/app/help/exoplayer/ExoPlayerHelper.kt` 的 `SNIFF_TIMEOUT_MS`
  - 从 `3000L` 恢复到 `5000L`
- [ ] 5.2 验证 m3u8 短路检测和 HTML 接口预判拦截不受影响
- [ ] 5.3 真机弱网场景验证嗅探成功率提升

## 6. HEAD 预检机制（维度9，新增，P0，FR-07）

- [ ] 6.1 新增 `app/src/main/java/io/legado/app/help/exoplayer/M3u8PreCheckDataSource.kt`
...
- [ ] 6.2 在 `ExoPlayerHelper.kt` 接入 M3u8PreCheckDataSource
  - 创建 HlsMediaSource 前调用 preCheck
  - 预检通过使用 finalUrl 创建 MediaItem
  - 预检失败跳过此候选 URL（如有多个候选）
- [ ] 6.3 验证 HEAD 预检逻辑正确性（200/302/403/失败降级）
- [ ] 6.4 真机验证 m3u8 播放失败率从 7.3% 降到 < 2%

## 7. Referer 请求头注入（维度6，新增，P0，FR-08）

- [ ] 7.1 在 `ExoPlayerHelper.kt` 新增 `buildAntiLeechHeaders()` 方法
...
- [ ] 7.2 修改 `cronetDataFactory` lazy 块
  - 调用 `setDefaultRequestProperties(buildAntiLeechHeaders())`
- [ ] 7.3 修改 `cacheDataSourceFactory` lazy 块
  - OkHttpDataSource.Factory 注入相同请求头
  - 设置 `setAllowCrossProtocolRedirects(true)` 保留 Referer
- [ ] 7.4 验证 Referer 注入正确性（日志只记录是否注入，不记录完整值）
- [ ] 7.5 真机验证 CDN 防盗链 403 错误减少 > 80%

## 8. AES-128 密钥请求注入（维度10，新增，P1，FR-09）

- [ ] 8.1 新增 `app/src/main/java/io/legado/app/help/exoplayer/HlsKeyDataSourceFactory.kt`
...
- [ ] 8.2 在 `ExoPlayerHelper.kt` 的 `createMediaSource` HLS 分支接入
  - 调用 `setKeySourceFactory(HlsKeyDataSourceFactory(appCtx, referer, BROWSER_UA))`
- [ ] 8.3 验证密钥请求注入正确性（日志只记录密钥长度和是否获取成功，不记录密钥内容）
- [ ] 8.4 真机验证加密 m3u8 播放成功率 > 90%

## 9. QUIC 协议启用（维度4，新增，P1，FR-10）

- [ ] 9.1 修改 `app/src/main/java/io/legado/app/lib/cronet/CronetHelper.kt` 的 `cronetEngine` lazy 块
...
- [ ] 9.2 验证 QUIC 配置正确性（CronetEngine 版本字符串包含 HTTP/3）
- [ ] 9.3 真机验证首帧延迟降低 33%（800ms vs 1200ms）
- [ ] 9.4 真机 4G/WiFi 切换验证连接迁移不断连

## 10. 302 重定向缓存（维度7，新增，P1，FR-11）

- [ ] 10.1 新增 `app/src/main/java/io/legado/app/help/http/RedirectCacheInterceptor.kt`
...
- [ ] 10.2 在 OkHttp 配置中接入 RedirectCacheInterceptor
  - 找到 OkHttpClient 构建处（okHttpClient.kt 或网络配置文件）
  - 添加 `addInterceptor(RedirectCacheInterceptor())`
  - 放在拦截器链前端（不与其他拦截器冲突）
- [ ] 10.3 验证缓存命中/未命中/过期/淘汰逻辑
- [ ] 10.4 真机验证同一 URL 不重复 302

## 11. ProGuard 规则补充（新增，P0，NFR-07）

- [ ] 11.1 修改 `app/proguard-rules.pro`
...
- [ ] 11.2 验证 release 包构建成功（R8 混淆通过）
- [ ] 11.3 验证 mapping.txt 无新增类被移除

## 12. 更新日志记录（P0，FR-06）

- [ ] 12.1 修改 `app/src/main/assets/updateLog.md` 记录本次修复内容
  - 基于 git diff 分析真实代码变更
  - 面向用户语言描述可感知变化
  - 包含：DoH 服务器优化、SO 下载源切换、HTTP/2 降级优化、嗅探超时恢复
  - 包含：HEAD 预检机制、Referer 注入、AES-128 密钥注入、QUIC 启用、302 重定向缓存
- [ ] 12.2 验证 updateLog.md 内容完整且无遗漏

## 13. 构建与验证

- [ ] 13.1 编译正式包（assembleAppRelease）验证无编译错误
- [ ] 13.2 验证 mapping.txt 无 Cronet 相关类被移除（保持 V3 修复成果）
- [ ] 13.3 验证 mapping.txt 无新增类被移除（M3u8PreCheckDataSource/HlsKeyDataSourceFactory/RedirectCacheInterceptor）
- [ ] 13.4 模拟器安装启动验证（无 SIGABRT/ANR/FATAL EXCEPTION）
- [ ] 13.5 真机 arm64 验证（核心场景）：
  - 首次安装打开视频订阅源不崩溃
  - DoH 解析成功率 > 80%（日志确认）
  ...
  - QUIC 协议启用（首帧延迟降低）
  - 302 重定向缓存命中（不重复跳转）
- [ ] 13.6 真机弱网场景验证嗅探成功率提升（从 60% 提升到 80%）
- [ ] 13.7 真机 4G/WiFi 切换验证 QUIC 连接迁移不断连

## 14. 文档同步

- [ ] 14.1 更新 docs/INDEX.md（移动到"✅ 已完成的功能"）
- [ ] 14.2 更新 .trae/memory/ai_memory_main.md（记录本次修复经验+新增 Hard Constraints）
- [ ] 14.3 检查是否需要更新 docs/project-rules/package-naming.md（SO 下载源变更）
- [ ] 14.4 清理临时文件和调试代码（Grep 确认无残留 android.util.Log.d/e）
