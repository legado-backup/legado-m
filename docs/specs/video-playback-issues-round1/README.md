...
> 基于 2026-07-12 用户实测日志深度分析，修复订阅源视频播放的 5 类问题。

## 功能概述

用户安装最新版客户端测试订阅源视频播放时，发现"原本使用网页拼接内置 video 标签的时候可以播放，但是改为使用内置视频播放器后，有些就播放不了了"。通过深度分析 `temp\Downloadslogs.(4)..zip` 日志（67个日志文件，07-12 当天播放失败 265 次），识别出 5 类问题需要修复优化。

## 核心能力

| 编号 | 问题 | 严重程度 | 修复方向 |
|------|------|---------|---------|
| P0 | ExoPlayer HLS 播放失败（ERROR_CODE_IO_UNSPECIFIED 2000） | P0 | 添加 WebView 降级机制 + 播放器类型配置 |
