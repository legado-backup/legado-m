...
> 总线修订 2026-08-31：R3 终测（4.1-4.5）已合并至 master-track-orchestration tasks 2.6（compose B0 真机合并窗口）执行，本文件仍为权威账本
...
- [x] 2.3.1 D1 P0 高频旧 View 弹框迁移 ComposeDialogFragment **收官（2026-09-01）**：✅ 全 ui/ 120+ 弹框基类逐一扫描复核，原清单 11 项（CacheChapter/ChangeBookSource/ChangeChapterSource/Dict/ChangeCover/FilePicker/SourceLogin/ReadSelectionImage/TipConfig/VerificationCode/OpenUrlConfirm）**已全部迁移完成**；剩余残留 View 弹框 7 个经批 A-F 逐批复核：WaitDialog/PhotoDialog/BottomWebViewDialog 登记保留（调用点耦合/全屏承载/WebView 特殊），ExploreFragment Kind 登记保留（源 JS 交互协议），SelectionWebSearchDialog 登记保留+专项建议入册 migration-registry；应迁尽迁达成
- [x] 2.3.2 D2 P0 系统 AlertDialog 高频点收敛 ComposeConfirmDialog 族 **收官（2026-09-01 批 E/F）**：✅ 累计转换约 **60 处**，覆盖上述 25 处基础上再新增批量文件——`FontSelectDialog`(单选×1) / `BookshelfTagManageActivity`(确认删除×1) / `ReadRecordDialog`(清空×1) / `AudioPlayActivity`(加入书架×1) / `WebViewActivity`(删源×1) / `RemoteBookActivity`(下载/重加书架×2) / `AiChatActivity`(删除消息/删助手/世界书多选/窗口Skill多选/窗口MCP多选/删历史/清历史×7，多选带 neutral「管理/清空」) / `ThemeManageActivity`(confirmDelete×1) / `SelectionSearchEngineManageDialog`(3字段表单+确认删除，alert 全清) / `HighlightRuleGroupManageDialog`(删分组×1，Compose 内 AppCompat helper) / `GroupEditDialog`(删分组×1) / `CoverCollectionDetailActivity`(删图×1) / `RssFragment`(删源×1) / `CacheManageActivity`(搜索输入→TextInput×1) / `VerificationCodeDialog`(删源×1) / `BookCharacterEditActivity`(在线头像/头像提示词→TextInput×2) / `BookCharacterManageActivity`(删角色×1) / `RssSourceEditActivity`(退出未保存×1) / `NavigationBarManageActivity`(confirmDelete×1) / `ReadRecordWidgetUi`(条目操作选择×1) / `AiImagePreviewDialog`(建组/重命名→TextInput、删图×3) / `ContentEditDialog`(章题编辑→TextInput×1)；删除类 dangerPositive=true 红色警示。**配套增强**：`ComposeMultiChoiceDialog` + `showComposeMultiChoiceDialog`(Fragment/Activity) 新增可选 `neutralText`/`onNeutral` 参数（向后兼容，存量调用零改动），使 AI 世界书/窗口 Skill/MCP 等多选弹框能保留「管理/清空」中性按钮。`compileAppDebugKotlin` + `assembleAppDebug` 双门禁多轮 BUILD SUCCESSFUL；测试包 **`legado_miss_app_3.26.082719.apk`** 含累计全部 D2 转换。**剩余 D2 多为 customView 复杂型**（AndroidAlertBuilder 大表单/进度滑块/勾选框持有 Dialog 引用等），需逐个专项 Compose 弹框逐文件登记推进。**进展（2026-09-01，master-track W0/W1 批次）**：✅ 盘点实况=全 ui/ 120+ 弹框基类逐一扫描，2.3.1 D1 P0 原清单 11 项（CacheChapter/ChangeBookSource/ChangeChapterSource/Dict/ChangeCover/FilePicker/SourceLogin/ReadSelectionImage/TipConfig/VerificationCode/OpenUrlConfirm）**已全部迁移完成**（本条目原"其余待迁 P0"描述陈旧）；✅ 批 A：CacheManageActivity 删除确认→ComposeConfirmDialog、ReadBookActivity 书库调试框→readOnly TextInput（简化说明：非滚动视图）、云端章节删除→TextInput+validateInput；✅ 批 B：ReadRecordComponentConfigDialog+ReadRecordRankDialog 换壳 ComposeDialogFragment、目标卡配置弹框登记部分迁移保留（头像 ActivityResult 延迟回写耦合）；提交 489d3aed7/098da583d（分支 feat/master-track-waves）。**剩余**：7 残留 View 弹框（WaitDialog/PhotoDialog/HighlightStyleDialog/PageKeyDialog/AdvancedTitleConfigDialog/SelectionWebSearchDialog/BottomWebViewDialog-特殊评估）+ D2 用点 3 处（ThemeManageActivity 双表单/ExploreFragment Kind/ReadBookActivity L3798 章节选择），分批方案 C-F 待推进。**进展（2026-09-01，批 C+D 复核）**：✅ 4 处逐一 Read 源码复核，**全部判定登记保留**（保守原则：等价迁移>登记保留，禁止破坏交互）——①**ThemeManageActivity L427/L460 双表单**=DialogThemePackageEditBinding 重度复合编辑器（4 Tab 分组+4 必选/8 可选颜色行（ColorPickerDialog）+3 图片裁剪行（ImageCropContract 回写）+面板背景模式+4 滑块行（圆角/布局/弹框/边框透明度）+卡片阴影/背景模糊数值行+双字体选择（FontSelectDialog）+双字体颜色+2 开关行+自定义宽高比尺寸+深度字体遍历应用），showComposeTextFormDialog 纯文本字段能力无法等价承载，等价迁移=主题编辑器整体 Compose 专项重写（20+ pendingXxx 状态字段+裁剪回写链路），非换壳批次范畴；②**WaitDialog**=Grep 实测 20 个调用文件（>8 阈值），其中 10 处 by lazy 长期持有 Dialog 引用+BaseBookshelfFragment setOnCancelListener 绑定协程取消（addBookJob.cancel）+AboutFragment 全局 helper 形态，DialogFragment 化需改 20 文件调用点且 by lazy 持有模式与 Fragment 生命周期不兼容、破坏取消语义，对齐 2.3.4"过渡期保留"先例；③**PhotoDialog**=全屏承载（setLayout(1f, MATCH_PARENT) 非卡片形态）+自研 PhotoView 手势缩放控件+ImageProvider 内存缓存/本地文件/网络三层回退加载链（OkHttpModelLoader sourceOriginOption）+13 处调用，对齐 D4"特殊承载类登记保留"先例。零代码修改，updateLog 不追加，基线编译门禁复核 BUILD SUCCESSFUL。**收官（2026-09-01 批 E/F）**：✅ 批 E：HighlightStyleDialog 内容已 Compose 仅基类未纳管→换 ReaderBottomSheetComposeDialogFragment（窗口透明/圆角/阅读菜单互斥/墨水屏基类纳管，删手写 onStart 背景）+AdvancedTitleConfigDialog 337 行 AlertDialog.setView 动态表单→ComposeDialogFragment+AppDialogFrame 全量重写（名称/正则开关/拆分规则/预览/高度因子 30..120/JSON 外链 CodeEdit 校验逐条等价）+调用方 AdvancedTitleManageActivity 改 showDialogFragment；✅ 批 F：书库云端章节选择→新建 LibraryCloudChapterDialog（分组 LazyColumn+行级读取/删除，替换 ReadBookActivity 约 130 行 View 动态代码）+PageKeyDialog 换壳（硬件翻页键拦截经 onPreviewKeyEvent 等价，调用方 BaseReadBookActivity/MoreConfigDialog 标准化）；ExploreFragment Kind 登记保留（源 JS 交互协议耦合）+SelectionWebSearchDialog 登记保留（WebView 承载+池化+sheet 联动，专项建议入册 migration-registry 六.4）；--rerun-tasks 全量编译 BUILD SUCCESSFUL+清场。**弹框迁移至此收官：全 ui/ 120+ 弹框基类盘点完毕，应迁尽迁/特殊场景全部登记留痕（5 家族收敛目标达成）**
