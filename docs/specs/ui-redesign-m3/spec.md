...
| FR-13 | **页面入口与功能操作完整性**（2026-09-07 AD-25 补）：每页交付必含 inbound/outbound 入口清单 + 操作保留/降级映射表（降级与删除同等待遇，须检查点裁决）；L2 场景覆盖 inbound 全集+outbound 每跳到达断言；批次开工前 diff 防呆校验（登记页集 vs 任务集，差集非空阻断） | P0 | AD-25 Decision 1-5 逐项核验 |
