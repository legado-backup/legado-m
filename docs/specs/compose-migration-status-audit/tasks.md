...
- [ ] 3.1 pages-inventory.md §0 替换为分册 §1.1 成品总览表（55% 行口径+双口径注释）
- [ ] 3.2 pages-inventory.md §G 按分册 §1.2 校正表 X-01~X-22 逐条执行（含 6 项"维持免改"核验）
- [ ] 3.3 ui-redesign-m3/tasks.md 头部冻结标注（分册 §1.4 成品段落）
- [ ] 3.4 migration-registry.md §七 登记块 aq~bn 24 项按分册 §1.3 粘贴（每项含证据三元组）
- [ ] 3.5 六.3 遗留销项：BookshelfItems GeneratedCover 归位裁决（分册 §1.5 裁决成品段：方案 A 迁 ThemeSpec 取色/B 登记豁免，建议 A，随 B2 S2 验收批次实施）
