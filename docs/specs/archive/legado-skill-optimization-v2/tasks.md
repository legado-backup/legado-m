...
| Layer | 任务数 | 状态 | 实际执行情况 |
|-------|--------|------|-------------|
| A 立即修复 | 5 | ✅ 全部完成 | A1 cf-bypass.md修复+A2陷阱#54修正+A3源码引用+A4 11文件错误模式修复+A5备份 |
| B 门禁强化 | 4 | ✅ 全部完成 | B1 Phase 0源码验证门禁+B2 JVM能力降级(3文件)+B3 source_ref规范+B4 Phase 4触发强化(4→6种) |
| C 架构瘦身 | 6 | ⏸️ 暂缓 | C0探查完成(发现legado_client被67文件引用含核心脚本debug-source.py)。C1-C3/C4/C6暂缓：legado_client是核心依赖不可归档，脚本合并工作量大收益有限 |
| D 经验引擎 | 3 | 🟡 部分完成 | D1 ✅ 清理basic-memory错误经验(删除3条+重写3条修正版+edit修正1条)+D2 ✅ 撤销机制文档。D3暂缓：增量补全source_ref是渐进改进非核心 |
| **总计** | **18** | **11 完成 / 7 暂缓** | **核心目标全部达成** |
...
| 目标 | 状态 | 证据 |
|------|------|------|
| ① 修复错误源头 | ✅ | cf-bypass.md 不再推荐 `loginUrl: @js:java.webView()` |
| ② 修复错误扩散 | ✅ | Grep 确认 11 个文件错误模式已修复 |
| ③ 建立写入门禁 | ✅ | Phase 0 源码验证门禁 + source_ref/verified_against_source 字段强制 |
| ④ JVM 能力诚实声明 | ✅ | 明确不覆盖 WebView/Activity/Cookie/真机网络栈 |
| ⑤ Phase 4 触发强化 | ✅ | 4→6 种失败条件 + 3 类强制触发场景（即使 JVM 测试通过） |
| ⑥ 清理 basic-memory 错误经验 | ✅ | 删除 3 条错误笔记 + 重写 3 条修正版 + edit 修正 1 条 |
| ⑦ 建立撤销机制 | ✅ | basic-memory-usage.md 新增"错误经验撤销流程"章节 |

### Layer C 暂缓理由

C4（归档 legado_client）不可行的根因：
- legado_client 被 **67 个文件引用**（Grep 确认）
- 含核心脚本 debug-source.py（端到端真机级调试首选，AI_README.md 明确）
- 归档会导致 30+ 脚本失效

C1-C3/C6（合并 60+ 脚本）暂缓理由：
- 合并后的脚本仍依赖 legado_client，瘦身效果有限
- 合并工作量大（需读取 16+ 原脚本）
- 与"干活效果"关系间接（脚本多 ≠ 干活差）

### D3 暂缓理由

D3（增量补全旧 references 的 source_ref）是渐进式改进：
- 核心错误经验已清理（D1）
- 新写入经验已有 source_ref 规范（B3）
- 旧 references 的 source_ref 补全可后续逐步进行
