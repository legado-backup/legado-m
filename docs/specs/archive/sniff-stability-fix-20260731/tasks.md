...
- [x] 1.1 读取当前 CronetInterceptor.kt + DohDns.kt + SSLHelper.kt + HttpHelper.kt + RedirectCacheInterceptor.kt 完整源码
...
- [x] 2.1 修改 RedirectCacheInterceptor.kt L67-84 响应处理逻辑
...
- [x] 2.2 验证 RedirectCacheInterceptor 命中缓存后走 Cronet
...
- [x] 3.1 在 CronetInterceptor.kt companion object 新增证书错误单独去重状态
...
- [x] 3.2 实现证书错误判定方法 isCertificateError()
...
- [x] 3.3 实现证书错误日志去重方法 logCertError()
...
- [x] 3.4 在 intercept() 异常处理集成证书错误降级（V3 关键：分支前置 return）
...
- [x] 4.1 在 DohDns.kt 新增公开方法 clearNegativeCache(hostname: String)
...
- [x] 4.2 实现 NAME_NOT_RESOLVED 判定方法 isNameNotResolvedError()
...
- [x] 4.3 在 intercept() 异常处理集成 NAME_NOT_RESOLVED 处理
...
- [x] 5.2 评估方案A影响清单（完全替换为 CronetTransport）
...
- [x] 6.1 新增独立常量 RECOVERY_PROBE_CHECK_INTERVAL_MS
...
- [x] 6.2 修改 intercept() 恢复探测触发检查使用新常量
...
- [x] 6.3 V3 移除 cronetEngineHealthy 标志位（V2 未实施，V3 确认不实施）
...
- [x] 7.2 评估 ProgressResponseBody 开销
  - 分析：进度回调对图片加载性能的影响
...
- [x] 7.3 评估 ReadManga.rateLimiter 限流影响
...
- [x] 8.1 编写 RedirectCacheInterceptor 单元测试
  - 测试项：302 缓存命中/未命中
...
- [x] 8.2 编写 CronetInterceptor 证书错误降级单元测试
...
- [x] 8.3 编写 DohDns clearNegativeCache 单元测试
  - 测试项：host级清理（非全清）
...
- [x] 9.1 编译测试包验证（io.legado.miss.app.debug）
...
- [x] 9.4 真机测试 NAME_NOT_RESOLVED 处理
...
- [x] 9.11 编译正式包+mapping.txt 检查（io.legado.miss.app.release）
