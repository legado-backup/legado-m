#!/usr/bin/env python3
r"""v5_4_debug_verify.py — 对 V5.4 最终 JSON 做 5 维度真机调试验证

5维度定义（同 v5_3_debug_verify.py）：
1. 域名维度：网络请求是否成功
2. 列表维度：ruleArticles 是否解析成功
3. 搜索维度：searchUrl + ruleArticles 搜索是否解析成功
4. 分类维度：sortUrl + ruleArticles 分类是否解析成功
5. 正文维度：ruleContent 是否解析成功

工作流（每个源）：
1. am start RssSourceDebugActivity --es key "源URL"
2. dump UI 找 textFl 触发分类维度调试
3. 清logcat → 等待 15秒 → 收集 logcat
4. 然后点击 textMy 触发搜索维度调试
5. 清logcat → 等待 15秒 → 收集 logcat
6. 分析5维度结果

输出：
- output/rss/v5_4_debug_verify_result.json
- output/rss/v5_4_debug_verify_report.md
- output/rss/v5_4_debug_shots/
- output/rss/v5_4_debug_logs/
...
JSON_PATH = r'f:\myself\github\WeAgentChat\temp\legado\output\rss\optimized_v5_4_final.json'
OUT_DIR = Path(r'f:\myself\github\WeAgentChat\temp\legado\output\rss')
SHOTS_DIR = OUT_DIR / 'v5_4_debug_shots'
LOGS_DIR = OUT_DIR / 'v5_4_debug_logs'
RESULT_JSON = OUT_DIR / 'v5_4_debug_verify_result.json'
REPORT_MD = OUT_DIR / 'v5_4_debug_verify_report.md'
...
def verify_one_source(idx, source_url):
    ...
    try:
        ...
        _, ui_xml = dump_ui(f'_tmp_debug_v5_4_{idx}.xml')
        if not ui_xml:
    ...
    try:
        start_debug_activity(source_url)
        time.sleep(ACTIVITY_LOAD_SEC)
        _, ui_xml = dump_ui(f'_tmp_debug_v5_4_{idx}_search.xml')
        if not ui_xml:
...
def generate_report(results, enabled_sources):
    ...
    md = []
    md.append('# v5_4 真机 5 维度调试验证报告')
    md.append('')
    md.append(f'**生成时间**: {time.strftime("%Y-%m-%d %H:%M:%S")}')
    md.append(f'**验证源数**: {total} 个 enabled=true 源')
    md.append(f'**数据来源**: `output/rss/optimized_v5_4_final.json`')
    md.append('')
    md.append('## 1. 调试统计')
    md.append('')
    ...
    md.append('')
    md.append('---')
    md.append('**说明**: 报告中源编号对应 `optimized_v5_4_final.json` 中 enabled=true 源的索引（0-based）。')
