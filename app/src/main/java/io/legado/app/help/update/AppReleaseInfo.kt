...
data class AppReleaseInfo(
    ...
    companion object {
         ...
         * legado_miss_app_{v}.apk → 版本号是最后一个 "_" 后的段；
 ...
 * legado_miss_app_debug_*（测试）/ legado_miss_app_*（正式）
 * 全部不含 "release"，导致两包全被解析成 OFFICIAL：
 * 测试包过滤永远为空（有新版也不提示），正式包会混入测试包甚至被"更新"成测试包。
 */
