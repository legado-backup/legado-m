    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        return try {
            ...
            if (isStreamResetException(e)) {
                ...
                // 铁证：日志时序 6608→6609→6610 证明 Canceled 在 onPause 之前，与 Activity 切换无关
                // E5 整改：chain.call().client() API 不可行（Call接口无client()方法）
                // 改用同模块 okHttpClient 的连接池清理（StreamResetRetryInterceptor 是 object 单例）
                try {
                    okHttpClient.connectionPool.evictAll()
                } catch (ignore: Exception) {}
                chain.connection()?.socket()?.close()
                // 重试一次（Call 未被 cancel，可正常执行）
                return try {
                    chain.proceed(request)
