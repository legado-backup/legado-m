...
import io.legado.app.ui.book.info.compose.BookInfoModernRoute
...
/**
 * 书籍详情页「现代样式」宿主（book-info-modern-compose AD-08）。
 *
 * 与沉浸式宿主 [BookInfoComposeActivity] 的关系：宿主样板（结果回调契约、刷新容器、
 * Composition 生命周期策略）同型对齐，但状态与 Route 完全独立，避免两样式共享
 * `uiState` 导致迭代互相污染。
 *
 * 页面构成：组件块序列顺序渲染（顺序与显隐由 [io.legado.app.help.config.BookInfoComponentConfig] 驱动）。
 */
class BookInfoModernActivity :
    ...
    // 现代样式页面额外消费的数据：完整章节列表（目录面板）与当前书籍（目录缓存状态判定）
    private var modernBook by mutableStateOf<Book?>(null)
    private var modernChapters by mutableStateOf<List<BookChapter>>(emptyList())
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        composeView.setContent {
            BookInfoModernRoute(
                ...
                book = modernBook,
                chapters = modernChapters,
                extraMenuActions = modernExtraMenuActions()
            )
        }
    ...
    private fun deleteBook() {
        viewModel.getBook()?.let { book ->
            if (LocalConfig.bookInfoDeleteAlert) {
                // 弹窗族收口：与沉浸式同口径（共享 Compose 确认框 + 本地书可选勾选「同时删除本地文件」）
                showComposeConfirmDialog(
                    title = getString(R.string.draw),
    ...
    /**
     * 经典页 `buildMenuActions` 中详情页独有的菜单项（分享 / 上传云端 / 置顶 / 拆分长章节 /
     * 日志 / 删除确认开关）。现代样式补齐这些项，保证菜单能力相对经典页不退化；
     * 顶栏与卡片已承载的跳转类动作仍走 [BookInfoActions]。
     */
    private fun modernExtraMenuActions(): List<MenuAction> {
        val book = viewModel.getBook(false)
        return buildList {
            add(MenuAction(title = getString(R.string.share), onClick = ::shareBook))
            add(MenuAction(title = getString(R.string.upload_to_remote), onClick = ::uploadBookToRemote))
            add(MenuAction(title = getString(R.string.to_top), onClick = { viewModel.topBook() }))
            if (book?.isLocalTxt == true) {
                add(
                    MenuAction(
                        title = getString(R.string.split_long_chapter),
                        checked = book.getSplitLongChapter(),
                        onClick = ::toggleSplitLongChapter
                    )
                )
            }
            add(MenuAction(title = getString(R.string.log), onClick = { showDialogFragment<AppLogDialog>() }))
            add(
                MenuAction(
    ...
    private fun shareBook() {
        ...
        // 视频/大体量书 JSON 可达数 MB（Binder 事务上限），超阈值降级为仅分享书源链接
        val shareStr = if (bookJson.length > MAX_SHARE_BOOK_JSON) {
            book.bookUrl
        } else {
            "${book.bookUrl}#$bookJson"
        }
        SourceCallBack.callBackBtn(
            this,
    ...
    private fun uploadBookToRemote() {
        val book = viewModel.getBook() ?: return
        book.getRemoteUrl()?.let {
            showComposeConfirmDialog(
                ...
                negativeText = getString(R.string.cancel),
                onPositive = { upLoadBook(book) }
            )
        } ?: upLoadBook(book)
    }

    private fun upLoadBook(book: Book, bookWebDav: RemoteBookWebDav? = AppWebDav.defaultBookWebDav) {
        lifecycleScope.launch {
    ...
    private fun toggleSplitLongChapter() {
        ...
        if (!book.getSplitLongChapter()) {
            toastOnUi(R.string.need_more_time_load_content)
        }
    }
}

/** 分享书籍 JSON 的长度上限（超限降级为仅分享链接，同经典页口径） */
