...
/**
 * 顶栏包 §1.2 路径 B：`StorageManageScreen` 的 `MenuAction → AppManagementMenuAction` 转换必须透传
 * **图标双源**（`icon` / `iconRes`）。
 */
class StorageManageOverflowIconResTest {

    @Test
    fun transformPassesBothIconSources() {
        val rel = "ui/book/storage/StorageManageScreen.kt"
