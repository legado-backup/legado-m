...
> **来源**：2026-07-26 用户决策"项目代码优化开发时必须打测试包完成测试，使用创建优化 skill 时的真机测试必须使用内置的正式包测试"

| 任务类型 | 必须使用的包 | 包名 | 说明 |
|---------|------------|------|------|
| **项目代码优化/开发** | 测试包 | `io.legado.miss.app.debug` | 代码变更后真机验证必须用测试包（debug 构建，含调试日志+未混淆+便于问题定位） |
| **书源/订阅源创建优化（Skill 真机测试）** | 内置正式包 | `io.legado.miss.app.release` | 使用 legado-source-creator skill 创建/优化源后的真机验证必须用正式包（release 构建，验证生产环境真实行为，含 ProGuard 混淆+签名一致性） |
| **共存包测试** | 共存包 | `io.legado.app.debug` | 仅在需要与原版 legado 共存场景下使用（如多 AI 并发测试隔离） |

### 选择理由

- **代码优化用测试包**：debug 构建保留完整调试信息、未启用 ProGuard 混淆、日志输出完整，便于定位代码变更引入的问题
- **Skill 真机测试用正式包**：release 构建模拟生产环境真实行为（含 ProGuard 混淆+正式签名+Cronet provider 保留验证），确保源在生产环境可用；同时验证 release 包是否因混淆规则缺失导致功能异常（铁证：Cronet 149+ ProGuard 规则缺失导致 release 包订阅源列表加载失败）
...
> **完整包名规范**：[package-naming.md](./docs/project-rules/package-naming.md)
