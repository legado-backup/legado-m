...
> 验证日期：2026-07-24（多线路多集按需采集 + R5 自动抓取已更新）
...
> 多线路多集增强（v3.26.072420+）：新增 ruleRoutes/ruleEpisodes 字段，按需采集替代 ruleContent 嵌套JSON全量采集
...
VideoPlay.startPlay() [VideoPlay.kt]
  │ source is RssSource
  │ ├─ ruleRoutes/ruleEpisodes 不为空?  → 多线路多集按需采集分支（v3.26.072420+）
  │ │   ├─ Rss.getRoutesAwait(ruleRoutes) → 采集线路名列表 → List<RssRoute>
  │ │   ├─ Rss.getEpisodesAwait(ruleEpisodes, routeIndex) → 采集当前线路集数（播放页URL）→ List<RssEpisode>
  │ │   └─ 用户切换集数时：VideoUrlExtractor.extractVideoUrlForEpisode(播放页URL)
  │ │      → 三层降级采集视频地址：①MacCMS播放页解析 ②DOM解析 ③WebView抓包
  │ │      → 注入 Referer → player.setUp(url)
  │ │
...
4. **多线路多集按需采集（v3.26.072420+）**：ruleRoutes 采集线路名列表，ruleEpisodes 采集集数列表（播放页URL），用户切换线路/集数时由 VideoUrlExtractor 按需采集视频地址（三层降级：MacCMS播放页解析→DOM解析→WebView抓包）
...
> **源作者编写指南**：ruleContent 三种格式（单URL/多行URL/JSON数组）+ 多线路多集按需采集字段（ruleRoutes/ruleEpisodes）的完整说明详见 [../special-scenarios/video-audio.md](../special-scenarios/video-audio.md) 5.6 节
