...
> **自进化沉淀闭环（AD-14）**：AI从源码阅读/陷阱修复/Playwright分析中发现新范式时，按质量标准（AD-15）沉淀到对应文档，实现 skill 越用越智能。
>
> **触发条件**：在源码阅读/陷阱修复/Playwright分析中发现新范式（如新的动态域名解析算法/新的反爬绕过方案/新的Rhino兼容性陷阱）。
>
> **沉淀流程**：
> 1. 发现新范式 → 2. 按质量标准抽象（通用范式+脱敏） → 3. 更新对应文档 → 4. 在本章节登记 → 5. 下次经验检索两源命中
>
> **质量标准（AD-15）**：
> - 通用范式抽象：标题按问题类型命名（如"动态域名解析""平衡括号算法"），禁止按站点命名
> - 脱敏案例：站点代号→通用描述，具体URL→路径模式（`/path/{id}`）
...
| 新的批量校验/死源清理经验 | troubleshooting/source-type-traps.md | 批量校验模式、死源清理流程 |
...
> 按时间倒序记录已沉淀的新经验，便于回溯与检索。每条经验已脱敏（站点代号/路径模式化）。
