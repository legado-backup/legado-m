...
> 本文档从 SKILL.md 拆分，包含 JVM 仿真器 + Python 客户端的代码进化机制完整说明。
...
| 触发场景 | 识别方式 | 示例 |
|---------|---------|------|
| JsExtensionsStub 缺少函数 | Phase 3 JVM 测试报错 `TypeError: java.xxx is not a function` | `java.getElements()` 未实现 |
| AnalyzeRule 解析 bug | Phase 3 JVM 结果与 Legado 实际结果不一致 | `tag.div!0@text` 索引计算错误 |
...
```
1. 记录进化需求 → basic-memory (note_type=experience, tags=["jvm-evolution"])
...
| `RuleEngineServer.kt` | 服务端入口 + 命令路由 | 需要新命令时 |
...
- [ ] 新函数是否需要 Android API？如是，是否可用 JVM 替代方案？

> **未实现函数速查**：当 JVM 测试报错 `TypeError: java.xxx is not a function` 时，查阅 `references/mock-unimplemented-functions.md` 获取该函数的影响级别（高/中/低）和建议处理方式（标记需真机/降级Python仿真/可补全实现）。
